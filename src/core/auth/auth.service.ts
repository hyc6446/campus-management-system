...
export class AuthService {
  ...
  async validateUser(
    email: string,
    password: string
  ): Promise<pt.USER_FULL_ROLE_DEFAULT_TYPE | null> {
    // 1.根据邮箱查找用户
    ...
    if (!isPasswordValid) {
      ...
      if (
        failUser &&
        failUser.failedLoginAttempts >= this.configService.get('auth.maxLoginAttempts')
      ) {
   ...
   * 验证JWT令牌
   *
   * 验证JWT令牌的有效性，检查令牌是否过期、用户是否存在、用户状态是否为激活、
   * 令牌是否与用户ID匹配、用户是否登出。验证失败直接抛出异常。
   *
   * @param token 待验证的JWT令牌
   * @returns 验证成功返回true
   * @throws AppException 当令牌无效或用户不存在时抛出
   */
  async validateToken(token: string): Promise<pt.USER_SAFE_ROLE_DEFAULT_TYPE> {
    
    // 1.验证令牌是否过期
    const verify: TokenPayload = await this.jwtService.verifyAsync(token, {
      secret: this.configService.get('auth.jwtSecret'),
    })

    if (verify && Number(verify.exp) < Date.now() / 1000) {
      throw new AppException('令牌已过期', 'TOKEN_EXPIRED', HttpStatus.UNAUTHORIZED)
    }
    // 2.验证用户是否存在
    const user = await this.userService.findByIdOptional(Number(verify.sub))
    if (!user) {
    ...
    // 4.验证令牌是否与用户ID匹配
    if (verify.sub !== user.id.toString()) {
      throw new AppException('令牌与用户ID不匹配', 'TOKEN_USER_MISMATCH', HttpStatus.FORBIDDEN)
    }
    // 5.验证用户是否登出
    const cacheToken = await this.redisService.hget(AUTH_CACHE_KEY(user.id), 'token')
    if (cacheToken === token) {
      throw new AppException('用户已登出', 'USER_LOGGED_OUT', HttpStatus.UNAUTHORIZED)
    }
